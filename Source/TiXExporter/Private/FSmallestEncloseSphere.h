...
inline void Givens(T& c, T& s, const T a, const T b)
{
	...
	else if (FMath::Abs(b) > FMath::Abs(a)) {
		const T t = a / b;
		s = 1 / sqrt(1 + t * t);
		c = s * t;
	}
...
public:
	...
	private:
		...
		TArray<bool> membership;      // S[i] in M iff membership[i]
	...
	private: // member fields for maintaining the QR-decomposition:
		FVector Q[3], R[3];	// (3 x 3)-matrices Q
	...
	public: // construction and deletion:
		...
		{
			check(IsMember(GlobalIndex(LocalIndex)) && Size() > 1);
		...
		T ShortestVectorToSpan(const FVector& P, FVector& OutW)
		{
		...
		void FindAffineCoefficients(const FVector& P, TArray<T>& Lambdas)
		{
	...
	FSmallestEncloseSphere(const TArray<FVector>& InPoints);
	~FSmallestEncloseSphere();
...
private:
	...
	TSubspan * Support;
	FVector CenterToAff;
	FVector CenterToPoint;
	T RadiusSquare, Radius;
	T DistToAff, DistToAffSquare;
};

template<typename T>
const T FSmallestEncloseSphere<T>::Eps = T(1e-14);

template<typename T>
FSmallestEncloseSphere<T>::FSmallestEncloseSphere(const TArray<FVector>& InPoints)
	: Points(InPoints)
{
	...
	{
		T Dist = FVector::DistSquared(InPoints[j], Center);
	...
	Support = new TSubspan(InPoints, Farest);
	...
	while (true)
	{
		...
		if (stopper >= 0 && Support->Size() <= 3) {
			...
			// update the radius
			const FVector& stop_point = Points[Support->AnyMember()];
			RadiusSquare = 0.f;
			RadiusSquare += FVector::DistSquared(stop_point, Center);
			Radius = sqrt(RadiusSquare);
		...
		++Iteration;
		//if (Iteration > Points.Num())
		//{
		//	bool print_points = false;
		//	if (print_points)
		//	{
		//		for (uint32 cp = 0; cp < (uint32)Points.Num(); ++cp)
		//		{
...
template<typename T>
FSmallestEncloseSphere<T>::~FSmallestEncloseSphere()
{
	if (Support != nullptr)
	{
		delete Support;
...
template<typename T>
bool FSmallestEncloseSphere<T>::SuccessfulDrop()
{
...
template<typename T>
T FSmallestEncloseSphere<T>::FindStopFraction(int32& stopper)
{
	...
	for (uint32 j = 0; j < (uint32)Points.Num(); ++j)
	{
		if (!Support->IsMember(j)) {
			...
			const T DirPointProd = CenterToAff | CenterToPoint;
			...
			bound -= CenterToPoint | CenterToPoint;
